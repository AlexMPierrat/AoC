...
function executeMove(crates, instruction){
    ...
    var from = parseInt(instruction[1])-1
    var to = parseInt(instruction[2])-1
    var slicedCrates = crates[from].slice(-number)
    var newline =crates[from].slice(0,-number)
    crates[to] =crates[to].concat(slicedCrates)
    crates[from] = newline
    return crates
}

function executeMove2(crates, instruction){
    var number = parseInt(instruction[0])
